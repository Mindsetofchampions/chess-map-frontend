...
import '../src/styles/storybook.css'; // Storybook-specific styles (matches repo structure)
...
const withReducedMotion = (Story: React.FC, context: any) => {
  // Override Framer Motion animations for consistent visual testing
  const MotionConfig: any = (motion as any)?.MotionConfig || React.Fragment;
...
export const parameters = {
  ...
  docs: {
    // Use a supported Storybook theme object to avoid color function errors
    theme: themes.dark,
    source: {
...
// Surface runtime errors prominently in the browser console for debugging
if (typeof window !== 'undefined') {
  window.addEventListener('error', (e) => {
    // eslint-disable-next-line no-console
    console.error('[Storybook GlobalError]', e.error || e.message, e);
  });
  window.addEventListener('unhandledrejection', (e) => {
    // eslint-disable-next-line no-console
    console.error('[Storybook UnhandledRejection]', e.reason || e);
  });
}

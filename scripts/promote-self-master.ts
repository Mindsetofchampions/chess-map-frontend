...
async function main() {
  ...
  const { error: loginError } = await s.auth.signInWithPassword({ email: MASTER_EMAIL, password: MASTER_PASSWORD });
  if (loginError) {
  ...
  console.log(JSON.stringify({ ok: !error, data, error: error ? { message: error.message, code: (error as any).code } : null }, null, 2));
}

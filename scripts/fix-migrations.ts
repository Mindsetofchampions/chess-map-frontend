...
function fixContent(
  ...
  out = out.replace(
    /create\s+type\s+if\s+not\s+exists\s+public\.(\w+)\s+as\s+enum\s*\(([\s\S]*?)\)\s*;/gi,
    (_m, name, vals) => {
      rules.push(2);
  ...
  out = out.replace(
    /(create\s+policy\s+["'][^"']+["']\s+on\s+[.\w]+\s+for\s+insert)([\s\S]*?);/gi,
    (_m, head, tail) => {
  ...
  out = out.replace(/create\s+policy\s+"([^"]+)"\s+on\s+([.\w]+)\s/gi, (m, pname, tbl) => {
    ...
    const dropBlock = [
      'do $plpgsql$ begin',
      "  if exists (select 1 from pg_policies where schemaname='" +
        schema +
        "' and tablename='" +
        table +
        "' and policyname='" +
  ...
  out = out.replace(
    /create\s+trigger\s+(\w+)\s+(.*?)\s+on\s+([.\w]+)\s/gi,
    (m, tname, mid, tbl) => {
...
function run() {
  try {
    ...
    for (const r of report) {
      ...
      lines.push(
        `- **${r.file}** → ${rulesApplied > 0 ? `rules applied: ${r.rules.join(', ')}` : 'no changes'}${warningsCount > 0 ? ` | warnings: ${warningsCount}` : ''}`,
      );

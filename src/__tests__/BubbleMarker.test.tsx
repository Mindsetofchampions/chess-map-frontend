...
describe('BubbleMarker', () => {
  ...
  it('applies correct size classes', () => {
    const { rerender } = render(<BubbleMarker {...defaultProps} size='sm' />);
  ...
  describe('Snapshot Tests', () => {
    ...
    it('matches snapshot for small size variant', () => {
      const { container } = render(<BubbleMarker {...defaultProps} size='sm' />);
      expect(container.firstChild).toMatchSnapshot();
    });

    it('matches snapshot for large size variant', () => {
      const { container } = render(<BubbleMarker {...defaultProps} size='lg' />);
      expect(container.firstChild).toMatchSnapshot();
    });

    it('matches snapshot with position and custom class', () => {
      const { container } = render(
        <BubbleMarker {...defaultProps} position={{ x: 50, y: 100 }} className='custom-class' />,
      );
      expect(container.firstChild).toMatchSnapshot();
    });
  });

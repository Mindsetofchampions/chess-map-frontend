...
describe('QuestPopup', () => {
  ...
  const mockQuest = {
    ...
    description: 'Complete wisdom-building activities with Hootie the Owl to develop strong character traits.',
    coins: 50,
  ...
  it('applies correct difficulty colors', () => {
    ...
    rerender(
      <QuestPopup 
        {...defaultProps} 
        quest={{ ...mockQuest, difficulty: 'easy' }}
      />
    );
    ...
    rerender(
      <QuestPopup 
        {...defaultProps} 
        quest={{ ...mockQuest, difficulty: 'hard' }}
      />
    );
  ...
  describe('Snapshot Tests', () => {
    ...
    it('matches snapshot for minimal quest data', () => {
      ...
      const { container } = render(
        <QuestPopup {...defaultProps} quest={minimalQuest} />
      );
      expect(container.firstChild).toMatchSnapshot();
    });
    ...
    it('matches snapshot for different difficulty levels', () => {
      ...
      const { container } = render(
        <QuestPopup {...defaultProps} quest={easyQuest} />
      );
      expect(container.firstChild).toMatchSnapshot();
    });
    ...
    it('matches snapshot when closed', () => {
      const { container } = render(
        <QuestPopup {...defaultProps} isOpen={false} />
      );
      expect(container.firstChild).toMatchSnapshot();
    });
  });

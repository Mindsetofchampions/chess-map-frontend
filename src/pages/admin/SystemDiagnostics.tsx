...
const DiagnosticCard: React.FC<DiagnosticCardProps> = ({
  ...
  const getStatusIcon = () => {
    ...
    switch (result.status) {
      ...
      case 'running':
        return <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-electric-blue-400" />;
    }
  };
        ...
        >
          <div className="flex items-center justify-between text-sm">
            <span className={`font-medium ${
              result.status === 'pass' ? 'text-cyber-green-400' :
...
const SystemDiagnostics: React.FC = () => {
  ...
  const checkQuests = useCallback(async () => {
    ...
    try {
      ...
      updateResult('quests', {
        ...
        data: { total: (data || []).length, approved: approved.length, submitted: submitted.length }
      });
    } catch (error: any) {
      updateResult('quests', {
        status: 'fail',
  ...
  }, [testQuestId, testChoiceId, updateResult, showSuccess, showError, showWarning, checkWalletLedger]);

  /**
   * Approval RPC Test
   */
  const runApprovalTest = useCallback(async (questId: string) => {
    ...
    try {
      ...
      const isPermissionError = error.message.includes('permission') || error.message.includes('master_admin');
  ...
  const checkSprites = useCallback(async () => {
    ...
    const promises = Object.entries(PERSONA_GIF).map(([key, src]) =>
    ...
    if (failed.length === 0) {
      updateResult('sprites', {
        ...
        details: Object.entries(spriteResults).map(([key, status]) => `${key}: ${status}`).join('\n')
      });
    } else {
      updateResult('sprites', {
        status: 'fail',
  ...
  const checkMap = useCallback(async () => {
    ...
    if (!mapboxToken || /YOUR_|example_/i.test(mapboxToken)) {
      updateResult('map', {
        ...
        details: 'VITE_MAPBOX_TOKEN not set or contains placeholder value. Map will show bubbles only.'
      });
      return;
    }
  ...
  }, [checkEnvironment, checkConnection, checkAuth, checkWalletLedger, checkQuests, checkSprites, checkMap, checkRoutes, showSuccess, showError]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-dark-primary via-dark-secondary to-dark-tertiary">
      <div className="container mx-auto max-w-7xl p-6">
        ...
        >
          ...
          <p className="text-gray-200 text-lg">Verify all system components and deployment readiness</p>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className={`text-sm ${
                readiness.ready ? 'text-cyber-green-200' : 'text-red-200'
              }`}>
                {readiness.ready 
          ...
          <GlassContainer variant="card">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-300 text-sm">Run individual tests or execute complete system verification</p>
              </div>
            ...
            <div className="space-y-4">
              ...
              {results.mcq && (
                <div className={`p-3 rounded-lg border ${
                  results.mcq.status === 'pass' ? 'bg-cyber-green-500/20 border-cyber-green-500/30' :
                  results.mcq.status === 'fail' ? 'bg-red-500/20 border-red-500/30' :
                  'bg-glass border-glass'
                }`}>
                  <p className={`text-sm font-medium ${
                    results.mcq.status === 'pass' ? 'text-cyber-green-300' :
                    results.mcq.status === 'fail' ? 'text-red-300' :
                    'text-gray-300'
                  }`}>
                    {results.mcq.message}
                  </p>
            ...
            <div className="space-y-4">
              {submittedQuests.length === 0 ? (
                <div className="text-center py-6">
                  ...
                  <p className="text-gray-300 text-sm">No submitted quests available for approval testing.</p>
                  <p className="text-gray-400 text-xs mt-1">Create a quest with status='submitted' to test approvals.</p>
                </div>
              ) : (
                <div className="space-y-3">
                  <p className="text-gray-200 text-sm">
                    {submittedQuests.length} quest{submittedQuests.length !== 1 ? 's' : ''} awaiting approval:
                  </p>
              ...
              {results.approvals && (
                <div className={`p-3 rounded-lg border ${
                  results.approvals.status === 'pass' ? 'bg-cyber-green-500/20 border-cyber-green-500/30' :
                  results.approvals.status === 'fail' ? 'bg-red-500/20 border-red-500/30' :
                  results.approvals.status === 'skip' ? 'bg-yellow-500/20 border-yellow-500/30' :
                  'bg-glass border-glass'
                }`}>
                  <p className={`text-sm font-medium ${
                    results.approvals.status === 'pass' ? 'text-cyber-green-300' :
                    results.approvals.status === 'fail' ? 'text-red-300' :
                    results.approvals.status === 'skip' ? 'text-yellow-300' :
                    'text-gray-300'
                  }`}>

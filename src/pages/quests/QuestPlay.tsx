...
import { ArrowLeft, Award, Clock, CheckCircle, XCircle, HelpCircle, Send } from 'lucide-react';
...
const QuestPlay: React.FC = () => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      if (submission.status === 'autograded') {
        ...
        showError(
          'Incorrect answer',
          "That's not quite right. Try reviewing the question and try again next time!",
        );
      }
    } catch (error: any) {
  ...
  const getPersonaInfo = (attributeId: string | null) => {
    ...
    return (
      personaMap[personaKey as keyof typeof personaMap] || {
        name: personaKey,
        emoji: '❓',
        color: 'text-gray-400',
      }
      ...
      <div className='min-h-screen bg-gradient-to-br from-dark-primary via-dark-secondary to-dark-tertiary flex items-center justify-center'>
        <GlassContainer variant='card' className='text-center max-w-md'>
          ...
          <p className='text-gray-300 mb-6'>
            The quest you're looking for doesn't exist or isn't available.
          </p>
          <button onClick={() => navigate('/quests')} className='btn-esports'>
            Back to Quests
          </button>
        </GlassContainer>
      </div>
        ...
        <GlassContainer variant='card'>
          ...
          >
            ...
            <div className='text-center mb-8'>
              ...
              {quest.description && (
                <p className='text-gray-200 text-lg max-w-2xl mx-auto'>{quest.description}</p>
              )}
                      ...
                      <div className='flex items-start gap-3'>
                        <div
                          className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mt-0.5 ${
                            selectedOption === option.id ? 'border-current' : 'border-gray-500'
                          }`}
                        >
                          {selectedOption === option.id && (
                        ...
                        <p className='font-medium leading-relaxed flex-1'>{option.text}</p>
                      </div>
                    </motion.button>
                  ))}
                </AnimatePresence>

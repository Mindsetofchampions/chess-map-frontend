...
import { Users, UserPlus, Mail, Crown, RefreshCw } from 'lucide-react';
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      ...
      const usersWithRoles = (data || []).map((profile: any) => ({
        id: profile.user_id,
        email: Array.isArray(profile.users)
          ? profile.users[0]?.email || 'Unknown'
          : profile.users?.email || 'Unknown',
        created_at: Array.isArray(profile.users)
          ? profile.users[0]?.created_at || profile.created_at
          : profile.users?.created_at || profile.created_at,
  ...
  const handleUpdateRole = useCallback(
    async (email: string, newRole: string) => {
      ...
      try {
        ...
        showSuccess('Role Updated', `Successfully updated ${email} to ${newRole}`);
              ...
              <form onSubmit={handleCreateUser} className='space-y-4'>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setCreateForm((prev) => ({ ...prev, role: e.target.value as any }))
                    }

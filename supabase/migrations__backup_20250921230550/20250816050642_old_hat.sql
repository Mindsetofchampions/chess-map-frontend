...
create policy "wallets: select own or staff+"
  on coin_wallets
  for select
  to public
  using (
...
create policy "ledger: select own or staff+"
  on coin_ledger
  for select
  to public
  using (
...
create policy "org_wallet_select_admins"
  on org_coin_wallets
  for select
  to authenticated
...
create policy "org_txn_select_admins"
  on org_coin_txns
  for select
  to authenticated
  using (actor_is_master_admin());
...
create policy "scw_select_self_or_org"
  on student_coin_wallets
  for select
  to authenticated
  using (
    user_id = auth.uid() 
...
create policy "sct_select_self_or_org"
  on student_coin_txns
  for select
  to authenticated
  using (
    user_id = auth.uid() 

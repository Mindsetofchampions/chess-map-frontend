...
create policy "platform_balance_master_update" on public.platform_balance 
  for update
  using (public.is_master_admin())
...
create policy "platform_ledger_master_insert" on public.platform_ledger 
  for insert
  with check (public.is_master_admin());
...
create policy "quests_creator_update_own" on public.quests 
  for update
  using (
...
create policy "quests_admin_update_any" on public.quests 
  for update

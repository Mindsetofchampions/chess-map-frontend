/**
 * Admin Delete Organization Edge Function
 *
 * Securely deletes an organization using service role after verifying
 * the caller is a master admin. Returns the deleted row.
 */
...
serve(async (req) => {
  ...
  try {
    ...
    const body: DeleteOrgRequest = await req.json();
    const id = body?.id;
    if (!id)
    ...
    console.error('admin_delete_org error', err);

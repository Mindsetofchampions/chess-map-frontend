...
serve(async (req) => {
  ...
  if (!RESEND_API_KEY && !SENDGRID_API_KEY) return new Response('mail provider API key not configured', { status: 500 })
    ...
    if (RESEND_API_KEY) {
      ...
      if (!res.ok) {
        const errText = await res.text()
        return new Response(`sendgrid error: ${errText}`, { status: 500 })
      }
    }

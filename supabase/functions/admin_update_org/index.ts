/**
 * Admin Update Organization Edge Function
 *
 * Securely updates an organization's status using service role
 * after verifying the caller is a master admin.
 */
...
serve(async (req) => {
  ...
  try {
    ...
    const body: UpdateOrgRequest = await req.json();
    const id = body?.id;
    ...
    console.error('admin_update_org error', err);
